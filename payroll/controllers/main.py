...
class HrPayrollCI(Controller):

    @route(["/print/payslips/gratification"], type='http', auth='user')
    def get_payroll_report_print(self, list_ids='', **post):
        if not request.env.user.has_group('payroll.group_hr_payroll_user') or not list_ids or re.search("[^0-9|,]", list_ids):
        ...
        payslip_reports = payslips._get_pdf_reports_custom()
